...
import { freshConfig, renameField } from './config.mjs';
...
<form id="reserve-form" method="post" action="/reserve" novalidate>
${inputs}
  <button type="submit">${esc(config.submitLabel)}</button>
</form>
</main>`,
  );
}
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/') return send(res, 200, formPage());

    if (req.method === 'POST' && url.pathname === '/reserve') {
      const body = new URLSearchParams(await readBody(req));
      const { errors, clean } = validate(body);
      if (Object.keys(errors).length) return send(res, 422, formPage(Object.fromEntries(body), errors));
      const reference = `HL-${randomBytes(4).toString('hex').toUpperCase().slice(0, 6)}`;
      reservations.set(reference, { reference, ...clean });
    ...
    if (url.pathname.startsWith('/_admin/')) {
      ...
      if (req.method === 'GET' && url.pathname === '/_admin/config') return sendJson(res, 200, config);
      ...
      if (req.method === 'POST' && url.pathname === '/_admin/break') {
        ...
        if (kind !== 'rename-field') return sendJson(res, 400, { error: `unknown break kind "${kind}"` });
        return sendJson(res, 200, { ...renameField(config, key), config });
      }
    }
